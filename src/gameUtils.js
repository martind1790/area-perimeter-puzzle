...
export function findLogicalStep(grid, puzzle) {
  ...
  const regionCounts = {};
  puzzle.regions.forEach(reg => { regionCounts[reg.id] = 0; });
  for (let r = 0; r < size; r++)
    for (let c = 0; c < size; c++)
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) {
      ...
      const candidates = puzzle.regions.filter(reg =>
  ...
  for (const reg of puzzle.regions) {
    ...
    for (let r = 0; r < size; r++)
      for (let c = 0; c < size; c++)
        if (grid[r][c] === reg.id) { visited.add(`${r},${c}`); queue.push([r, c]); }
    ...
    while (qi < queue.length) {
      ...
      for (const [dr, dc] of dirs) {
        ...
        if (nr >= 0 && nr < size && nc >= 0 && nc < size && !visited.has(key)) {
          visited.add(key);
          if (grid[nr][nc] === -1) { reachable.push([nr, nc]); queue.push([nr, nc]); }
        }
      }
    }
